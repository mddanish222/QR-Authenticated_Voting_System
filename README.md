...
| Security | JWT verification, RLS policies, rate limiting, OTP verification |
...
### Core Flow
1. Student scans a QR / enters ID on `qr.html` → backend resolves eligibility (`/api/verify/student`)
2. First-time voters are routed to `blink.html` to capture a face descriptor
3. Returning voters land on `home.html`, authenticated by descriptor match, and cast a vote
4. Admin can publish results for a separate time window on `publish.html`; eligible students view them on `result.html`
